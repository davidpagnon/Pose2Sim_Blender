...
def show_images(camera, img_vid_path, single_image=False, frame_offset=0):
    ...
    # add scale factor and camera shifts as custom properties on img
    img_size_orig = img.empty_display_size
    scale_factor = img_size_m / img_size_orig
    
    img['scale_factor'] = scale_factor
    img['shift_x'] = camera.data.shift_x
    img['shift_y'] = camera.data.shift_y
    ...
    d_scalex.driver.expression = f'-scaleX * self["scale_factor"]'
    d_scalex.driver.use_self = True
    ...
    d_scaley.driver.expression = f'-scaleY * self["scale_factor"]'
    d_scaley.driver.use_self = True
    ...
    d_locx.driver.expression = 'self["shift_x"] * locX' # no minus signe because camera flip along X
    d_locx.driver.use_self = True
    ...
    d_locy.driver.expression = '-self["shift_y"] * locY'
    d_locy.driver.use_self = True
